import { miGraphService } from "./database";
import {
  BOMNode,
  BOMEdge,
  AlternateGroup,
...
export class MISeedDataGenerator {
  ...
  private async createAircraftNode(): Promise<BOMNode> {
    ...
    return await miGraphService.createNode(aircraft);
  }
  ...
  private async createEngineSystem(aircraftId: string): Promise<BOMNode> {
    ...
    const engineNode = await miGraphService.createNode(engine);
    ...
    await miGraphService.batchWrite([
  ...
  private async createCombustionSection(engineId: string): Promise<BOMNode> {
    ...
    const combustionNode = await miGraphService.createNode(combustion);
  ...
  private async createCombustionModule(
    ...
    const combustionModule: Omit<BOMNode, "created_at" | "updated_at"> = {
    ...
    const moduleNode = await miGraphService.createNode(combustionModule);
  ...
  private async createSubComponents(moduleId: string): Promise<void> {
    ...
    await miGraphService.batchWrite([...nodes, ...edges]);
  }
  ...
  private async createSuppliers(moduleId: string): Promise<void> {
    ...
    await miGraphService.batchWrite(supplierLinks);
  }
  ...
  private async createAlternates(moduleId: string): Promise<void> {
    ...
    await miGraphService.createNode(alternateNode);
    await miGraphService.batchWrite([
      alternateGroup as unknown as Record<string, unknown>,
      alternateGroupMember as unknown as Record<string, unknown>,
    ]);
  }
  ...
  private async createSupersession(): Promise<void> {
    ...
    await miGraphService.createNode(oldPart);
    await miGraphService.batchWrite([
      supersession as unknown as Record<string, unknown>,
    ]);
  }
  ...
  private async createTailInstallations(moduleId: string): Promise<void> {
    ...
    await miGraphService.batchWrite([...effectivities, ...installations]);
  }
  ...
  private async createTechnicalOrders(moduleId: string): Promise<void> {
    ...
    await miGraphService.batchWrite([...docMeta, ...docRefs]);
  }
  ...
  private async createForecastData(): Promise<void> {
    ...
    await miGraphService.createForecast(forecast);
  }
  ...
  private async createWorkbenchIssue(moduleId: string): Promise<void> {
    ...
    await miGraphService.createIssue(issue);
    await miGraphService.batchWrite([
  ...
  private async createContentBlocks(): Promise<void> {
    ...
    await miGraphService.batchWrite(contentBlocks);
  }
