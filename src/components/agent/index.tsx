...
export function AgentProvider({ 
  ...
  // API hooks
  const { execute: executeAgent } = useApi(async (client, operation: string, data?: any) => {
    return client.run({
      service: 'agent',
      operation,
      app: 'core',
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      if (result && result.success && result.data) {
        ...
        setThreads(prev => prev.map(t => 
          t.id === currentThread.id ? thread : t
        ));
        setCurrentThread(thread);
      }
    } catch (error) {
  ...
  const refreshThreads = useCallback(async () => {
    try {
      ...
      if (Array.isArray(result)) {
        ...
        console.error('[AgentProvider] API call failed - executeAgent returned null');
        setThreadsError('Failed to load chat history');
      } else {
        console.error('[AgentProvider] Unexpected result type:', typeof result, result);
        setThreadsError('Failed to load chat history');
      }
    } catch (error) {
