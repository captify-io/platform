...
function createSimpleDeploymentPackage() {
  ...
  if (existsSync(deployDir)) {
    runCommand(`rmdir /s /q "${deployDir}"`, 'Cleaning previous deployment package');
  }
  mkdirSync(deployDir);
  ...
  writeFileSync(join(deployDir, 'package.json'), JSON.stringify(simplePackageJson, null, 2));
...
async function main() {
  try {
    ...
    const accountInfo = runCommandSilent(`aws sts get-caller-identity --query "Account" --output text --region ${REGION}`);
    if (!accountInfo) {
    ...
    if (!existsSync('.next')) {
      console.error('❌ .next folder not found. Please run "pnpm run build" first.');
      process.exit(1);
    }
    ...
    console.log(`🌐 Check your application at: http://anautics-ai.us-east-1.elasticbeanstalk.com`);
