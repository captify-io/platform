...
function createDeploymentPackage() {
  ...
  filesToCopy.forEach(file => {
    if (existsSync(file)) {
      ...
      if (stats.isDirectory()) {
        runCommand(`xcopy "${file}" "${deployDir}\\${file}" /E /I /H /Y`, `Copying ${file} directory`);
      } else {
  ...
  if (existsSync('.env.local')) {
    runCommand(`copy ".env.local" "${deployDir}\\.env"`, 'Copying environment file');
  }
...
async function main() {
  try {
    ...
    const envCheck = runCommandSilent(`aws elasticbeanstalk describe-environments --environment-ids ${ENV_ID} --region ${REGION} --query "Environments[0].EnvironmentName" --output text`);
