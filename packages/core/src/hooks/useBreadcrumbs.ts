...
import type { NavigationBreadcrumbItem } from "../context/CaptifyContext";

/**
 * Hook to customize breadcrumbs for specific pages
 * Usage: useBreadcrumbs([{ label: "Custom Page", href: "/custom" }])
 */
export function useBreadcrumbs(customBreadcrumbs?: NavigationBreadcrumbItem[]) {
  const { setBreadcrumbs } = useCaptify();
 ...
 */
export function useContextualBreadcrumb(item: NavigationBreadcrumbItem) {
  const { addBreadcrumb } = useCaptify();
...
export function useAutomaticBreadcrumbs(enabled: boolean = true): {
  ...
  const { appData, appLoading, slug, setBreadcrumbs } = useCaptify();

  useEffect(() => {
    if (!enabled || appLoading || !pathname) return;
    ...
    if (appData) {
      breadcrumbs.push({
        label: appData.name || "Application",
        href: `/${slug}`,
      });
    }
  ...
  }, [enabled, appData, pathname, slug, appLoading, setBreadcrumbs]);

  return { loading: appLoading, slug };
}
