...
import { ThreadPanelHeader } from "../client/components/ThreadPanelHeader";
import { ContextPanel } from "../client/components/ContextPanel";
import { ToolsPanel } from "../client/components/ToolsPanel";
import { ReasoningPanel } from "./ReasoningPanel";
import { useThreads, type ChatApiClient } from "../client/hooks/useThreads";
import { useCaptify } from "@captify/client";
import { Settings } from "lucide-react";

// Temporary stub API client - should be replaced with actual implementation
const stubApiClient: ChatApiClient = {
  getThreads: async () => ({ threads: [] }),
  getAgents: async () => ({ agents: [], defaultAgentId: null }),
  createThread: async () => ({ threadId: `thread-${Date.now()}` }),
  renameThread: async () => {},
  deleteThread: async () => {},
  pinThread: async () => {},
};
...
export function ConsoleLayout({
  ...
  const { isMenuVisible, setHasMenu, appData, appLoading } = useCaptify();
  ...
  const {
    ...
    autoLoad: false, // Disable auto-load for now
  });
